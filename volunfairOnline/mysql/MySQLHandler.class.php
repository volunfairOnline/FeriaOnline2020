...
function OpenConnection($quien = 'PequeLiante')   {
    ...
    if ($this->USE_PERMANENT_CONNECTION) {
        $conn = mysqli_pconnect($servidor, $usuario, $contrasena, "LaLiamos") or die(mysqli_error($conn));
		...
		$conn = mysqli_connect($servidor, $usuario, $contrasena, "LaLiamos") or die(mysqli_error($conn));
    ...
    if ((!$conn) || (!mysqli_select_db($conn, 'LaLiamos' ))) { // --- Para que funcione con MySQLi, hay que o ponerlo como objeto o as�
